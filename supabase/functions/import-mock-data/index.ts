...
// This function is designed to import the initial mock data into Supabase
// It can be triggered manually or scheduled
...
const mockOutfits: Outfit[] = [
  ...
  },
  {
    ...
    description: "Timothée makes a statement in a custom Haider Ackermann suit with modern proportions and unconventional details.",
    fullDescription: "Timothée Chalamet commanded attention at the Cannes Film Festival in this masterfully tailored Haider Ackermann suit. The design features slightly cropped trousers and a reconstructed blazer with subtle asymmetric elements that challenge traditional menswear proportions. The rich texture of the fabric adds depth to the monochromatic palette, while minimal styling lets the impeccable tailoring speak for itself. This appearance reinforces Timothée's position as one of the most exciting figures in men's fashion, consistently bringing a fresh perspective to classic formal wear.",
...
const mockAffiliateProducts: AffiliateProduct[] = [
  {
    ...
    description: "Channel Zendaya's red carpet glamour with this stunning designer-inspired gown."
  },
  {
    id: "product-2",
    outfitId: "outfit-1",
    ...
    description: "Complete your red carpet look with these dazzling statement earrings."
  },
  {
    id: "product-3",
    outfitId: "outfit-1",
  ...
  },
  {
    ...
    retailer: "Amazon",
    affiliateLink: "https://amazon.com",
    description: "Stay warm and stylish with this Rihanna-inspired oversized coat."
  },
  {
    id: "product-5",
    outfitId: "outfit-2",
    ...
    retailer: "Myntra",
    affiliateLink: "https://myntra.com",
    description: "These trendy wide-leg jeans are perfect for an off-duty look."
  }
];
...
serve(async (req) => {
  ...
  try {
    // Create a Supabase client with the Service Role API key
    const supabaseClient = createClient(
      Deno.env.get("SUPABASE_URL") ?? "",
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? ""
    );

    console.log("Importing mock data into Supabase...");
    
    // Clear existing data (optional)
    await supabaseClient.from('affiliate_products').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabaseClient.from('outfit_tags').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabaseClient.from('outfits').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabaseClient.from('blog_posts').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    await supabaseClient.from('celebrities').delete().neq('id', '00000000-0000-0000-0000-000000000000');
    
    console.log("Existing data cleared...");

    // Insert celebrities
    console.log("Inserting celebrities...");
    const { error: celebError } = await supabaseClient.from('celebrities').insert(
      mockCelebrities.map(celeb => ({
        ...
        style_type: celeb.styleType,
        outfitCount: celeb.outfitCount
      }))
    );
    
    if (celebError) {
      console.error("Error inserting celebrities:", celebError);
      throw new Error(`Error inserting celebrities: ${celebError.message}`);
    }

    // Insert outfits
    console.log("Inserting outfits...");
    const { error: outfitError } = await supabaseClient.from('outfits').insert(
      mockOutfits.map(outfit => ({
        id: outfit.id,
        title: outfit.title,
        description: outfit.description,
        image: outfit.image,
        full_description: outfit.fullDescription || null,
        occasion: outfit.occasion || null, 
        date: outfit.date ? new Date(outfit.date).toISOString().split('T')[0] : null,
    ...
    if (outfitError) {
      console.error("Error inserting outfits:", outfitError);
      throw new Error(`Error inserting outfits: ${outfitError.message}`);
    }

    // Insert blog posts
    console.log("Inserting blog posts...");
    const { error: blogError } = await supabaseClient.from('blog_posts').insert(
    ...
    if (blogError) {
      ...
      throw new Error(`Error inserting blog posts: ${blogError.message}`);
    }

    // Insert affiliate products
    console.log("Inserting affiliate products...");
    const { error: productError } = await supabaseClient.from('affiliate_products').insert(
      mockAffiliateProducts.map(product => ({
        id: product.id,
        outfit_id: product.outfitId,
        title: product.title,
        price: product.price,
        image: product.image,
    ...
    if (productError) {
      console.error("Error inserting affiliate products:", productError);
      throw new Error(`Error inserting affiliate products: ${productError.message}`);
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        stats: {
          celebrities: mockCelebrities.length,
          outfits: mockOutfits.length,
          blogPosts: mockBlogPosts.length,
          affiliateProducts: mockAffiliateProducts.length
        }
      }),
      {
        headers: {
