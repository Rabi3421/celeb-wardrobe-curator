...
const mockOutfits: Outfit[] = [
  {
    ...
    celebrity: "Zendaya",
    description:
      "Zendaya's stunning outfit from the Met Gala that turned heads.",
    fullDescription:
      "This elaborate ensemble designed by a top fashion house perfectly embodied the gala's theme while showcasing Zendaya's fashion-forward sensibilities.",
    occasion: "Red Carpet",
    date: "2024-05-06",
  ...
  },
  {
    ...
    fullDescription:
      "This custom outfit designed for the opening night showcased Harry's unique style while allowing for maximum movement during performances.",
    occasion: "Concert",
    date: "2024-04-15",
...
const mockBlogPosts: BlogPost[] = [
  {
    ...
    excerpt:
      "Exploring how Zendaya has changed the landscape of celebrity fashion in recent years.",
    content:
      "Zendaya has consistently pushed boundaries with her red carpet choices...",
    image:
    ...
    meta_description:
      "How Zendaya revolutionized red carpet fashion with her bold choices",
    structured_data: "",
...
const mockAffiliateProducts: AffiliateProduct[] = [
  {
    ...
    outfitId: "1",
    image:
      "https://images.unsplash.com/photo-1619096252214-ef06c45683e3?ixlib=rb-1.2.1&auto=format&fit=crop&w=800&q=80",
...
export const generateUniqueCelebritySlug = async (
  name: string,
  excludeId?: string
): Promise<string> => {
  try {
    ...
    const existingCelebrity = mockCelebrities.find(
      (c) => c.slug === baseSlug && (!excludeId || c.id !== excludeId)
    );
    ...
    while (
      mockCelebrities.find(
        (c) => c.slug === uniqueSlug && (!excludeId || c.id !== excludeId)
      )
    ) {
      counter++;
...
export const generateUniqueOutfitSlug = async (
  title: string,
  excludeId?: string
): Promise<string> => {
  try {
    ...
    // Check if slug already exists (mock implementation)
    const existingOutfit = mockOutfits.find(
      (o) => o.slug === baseSlug && (!excludeId || o.id !== excludeId)
    );
    ...
    while (
      mockOutfits.find(
        (o) => o.slug === uniqueSlug && (!excludeId || o.id !== excludeId)
      )
    ) {
      counter++;
      uniqueSlug = `${baseSlug}-${counter}`;
...
export const getCelebrityById = async (
  id: string
...
export const fetchCelebrityBySlug = async (
  ...
  try {
    ...
    const response = await axios.get(
      `${API_CONFIG.baseUrl}/celebrities/${slug}`,
      {
        headers: {
          "Content-Type": "application/json",
          api_key: API_CONFIG.websiteApiKey,
...
export const addCelebrity = async (
  celebrity: Partial<Celebrity>
  ...
  try {
    ...
    const slug =
      celebrity.slug ||
      (await generateUniqueCelebritySlug(celebrity.name || ""));
...
export const addOutfit = async (
  ...
  try {
    ...
    const newOutfit = {
      ...
      celebrity:
        mockCelebrities.find((c) => c.id === outfit.celebrityId)?.name || "",
    } as Outfit;
...
export const fetchOutfitBySlug = async (
  slug: string
): Promise<Outfit | null> => {
  try {
...
export const fetchAffiliateProductsByOutfitId = async (
  outfitId: string
): Promise<AffiliateProduct[]> => {
  try {
    console.log("Fetching affiliate products by outfit ID:", outfitId);
    await new Promise((resolve) => setTimeout(resolve, 200));
    return mockAffiliateProducts.filter(
      (product) => product.outfitId === outfitId
    );
  } catch (error) {
